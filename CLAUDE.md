...
- **`reqwest` TLS backend**: `Cargo.toml` pins `reqwest` with `default-features = false` and selects `rustls-tls` (re-listing `http2`/`charset`/`system-proxy` to keep the other defaults). The default `native-tls` feature drags in `openssl-sys`, which has no system OpenSSL to link in the musl cross-build `release.yml` uses, so it broke the static Linux binaries. rustls also unifies the TLS stack with `aws-sdk-sts` on rustls 0.23 (same ADR 0002). Do not switch reqwest back to `default-tls`/`native-tls` without re-checking the musl release build.
